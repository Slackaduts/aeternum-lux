...
function cull_instances(_buffer = 250, _exclusions = [yui_document, objCamera, objMainMenu]) {
	...
	for (var _index = 0; 0 < array_length(instance_id); _index++) {
		...
		if item_index(_exclusions, _inst.object_index) == undefined && variable_instance_exists(_inst, "canDeactivate") && _inst.canDeactivate {
